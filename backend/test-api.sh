...
HEALTH_RESPONSE=$(curl -s http://localhost:3005/health | jq .)
...
echo '$ curl -X POST -H "Content-Type: application/json" -d @/tmp/sample-request.json http://localhost:3005/ask | jq .'
echo ""
echo "Note: You'll need to update the sample request with valid Oracle credentials."
echo "The API is now ready for use at http://localhost:3005"